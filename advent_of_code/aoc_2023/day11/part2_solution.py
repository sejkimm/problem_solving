...
def get_expanded_universe(universe: List[List[str]]) -> List[List[str]]:
    ...
    for row in universe:
        ...
        if all([cell == "." or cell == "*" for cell in row]):
            expanded_universe.append(row)
    ...
    for col in transposed_universe:
        ...
        if all([cell == "." or cell == "*" for cell in col]):
            expanded_transposed_universe.append(col)
...
def solve(input_lines: List[str]) -> int:
    ...
    for row_idx, row in enumerate(universe):
        if all([cell == "." for cell in row]):
            expanded_rows.add(row_idx)

    for col_idx, col in enumerate(zip(*universe)):
        if all([cell == "." for cell in col]):
            expanded_cols.add(col_idx)

    galaxies: List[Tuple[int, int]] = [
        (row_idx, col_idx) for row_idx, row in enumerate(universe) for col_idx, elem in enumerate(row) if elem == "#"
    ]
    ...
    for idx, galaxy in enumerate(galaxies):
        ...
        for other_galaxy in other_galaxies:
            intermediate_rows: Set[int] = set(
                range(
                    min(galaxy[0], other_galaxy[0]) + 1 if galaxy[0] != other_galaxy[0] else galaxy[0],
                    max(galaxy[0], other_galaxy[0]),
                )
            )

            intermediate_cols: Set[int] = set(
                range(
                    min(galaxy[1], other_galaxy[1]) + 1 if galaxy[1] != other_galaxy[1] else galaxy[1],
                    max(galaxy[1], other_galaxy[1]),
                )
            )

            answer += (
                abs(galaxy[0] - other_galaxy[0])
                + (len(intermediate_rows.intersection(expanded_rows)) * (UNIVERSE_AGE - 1))
                + abs(galaxy[1] - other_galaxy[1])
                + (len(intermediate_cols.intersection(expanded_cols)) * (UNIVERSE_AGE -1 ))
            )
